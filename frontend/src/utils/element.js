...
// Path2D objects can't survive JSON (over REST or Socket.IO) — they
// serialize down to "{}". BRUSH elements need their `path` rebuilt from
// `points` after every load/sync. Other element types (LINE, RECTANGLE,
// CIRCLE, ARROW, TEXT) use plain serializable data, so they pass through
// untouched. Shared by CanvasPage (initial load) and BoardProvider
// (live drawingUpdate events from other users).
export const rebuildElements = (elements = []) => {
